...
export default async function handler(
  ...
  try {
    ...
    const result =
      await sql`SELECT Repository, Clicks FROM ExtensionClicks WHERE Repository=${repo}`;
    if (result.rows.length === 1) {
      const row = result.rows[0];
      const newClickCount = row["clicks"] + 1;
      await sql`UPDATE ExtensionClicks SET Clicks=${newClickCount} WHERE Repository=${repo}`;
